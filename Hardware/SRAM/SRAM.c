...
//  GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化 //PB15 推挽输出,控制背光
  ...
  GPIO_PinAFConfig(GPIOG,GPIO_PinSource10,GPIO_AF_FSMC);
 	...
 	readWriteTiming.FSMC_AddressSetupTime = 2;	 //地址建立时间（ADDSET）为1个HCLK 1/168M
  readWriteTiming.FSMC_AddressHoldTime = 0x00;	 //地址保持时间（ADDHLD）模式A未用到	
  readWriteTiming.FSMC_DataSetupTime = 6;		 ////数据保持时间（DATAST）为9个HCLK 16*9	 	 
...
{
    EXT_SRAM16[Index] = data;
}
uint16_t SRAM_Read_Halfword(uint32_t Index)
{
    return EXT_SRAM16[Index];
}
