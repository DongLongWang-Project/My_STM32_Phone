...
/** 
  * @brief  以下命令是 SD 卡特定命令。
  *         应在发送这些命令之前发送 SDIO_APP_CMD。
  */
#define SDIO_SEND_IF_COND               ((uint32_t)0x00000008)



/** @defgroup STM324xG_EVAL_SDIO_SD_私有变量
  * @{
  */
...
/** @defgroup STM324xG_EVAL_SDIO_SD_私有函数原型
  * @{
  */
...
/**
  * @brief  初始化SD卡使其转为就绪状态
  * @param  无
  * @retval SD_Error: SD卡的错误代码
  */
SD_Error SD_Init(void)
{
  ...
  /*!< 在 STM32F4xx 器件上，SDIOCLK 固定为 48MHz */
...
* @brief 返回当前卡的状态。
  * @参数无
  * @retval SDCardState：SD 卡错误或 SD 卡当前状态。  */
...
/*!< 开机顺序----------------------------------------------------*/
  ...
  /*!< 在 STM32F4xx 器件上，SDIOCLK 固定为 48MHz */
  /*!< 用于初始化的 SDIO_CK 不应超过 400 KHz */  SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV;
  SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising;
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;
...
/**
  * @brief 返回有关特定卡的信息。
  * @param cardinfo：指向包含所有SD卡的SD_CardInfo结构体的指针 
  * 信息。
  * @retval SD_Error：SD 卡错误代码。
  */
...
/**
  * @brief 如果支持的话，为请求的卡启用宽总线操作 
  *卡。
  * @param WideMode：指定SD卡宽总线模式。 
  * 该参数可以是下列值之一：
  * @arg SDIO_BusWide_8b：8位数据传输（仅适用于MMC）
  * @arg SDIO_BusWide_4b：4位数据传输
  * @arg SDIO_BusWide_1b：1位数据传输
  * @retval SD_Error：SD 卡错误代码。
  */
...
/**
  * @brief 允许从卡中的指定地址读取一个块。数据
  * 传输可以通过 DMA 模式或轮询模式进行管理。 
  * @note 此操作后面应该有两个函数来检查是否 
  * DMA 控制器和 SD 卡状态。
  * - SD_ReadWaitOperation()：该函数确保 DMA
  * 控制器已完成所有数据传输。
  * - SD_GetStatus(): 检查SD卡是否完成 
  * 数据传输并准备好数据。            
  * @param readbuff：指向包含接收数据的缓冲区的指针
  * @param ReadAddr：要读取数据的地址。  
  * @param BlockSize：SD卡数据块大小。块大小应为 512。
  * @retval SD_Error：SD 卡错误代码。
  */
...
/**
  * @brief 允许从卡中的指定地址开始写入一个块。
  * 数据传输可以通过 DMA 模式或轮询模式进行管理。
  * @note 此操作后面应该有两个函数来检查是否 
  * DMA 控制器和 SD 卡状态。
  * - SD_ReadWaitOperation()：该函数确保 DMA
  * 控制器已完成所有数据传输。
...
/**
  * @brief 该函数等待 SDIO DMA 数据传输完成。 
  * 该函数应该在SDIO_WriteBlock()之后调用并且
  * SDIO_WriteMultiBlocks()函数确保所有发送的数据 
  * 卡已由 DMA 控制器传输。        
  * @参数无。
  * @retval SD_Error：SD 卡错误代码。
  */
...
/**
  * @brief 返回当前卡的状态。
  * @param pcardstatus：指向将包含 SD 卡的缓冲区的指针 
  * 状态（卡状态寄存器）。
  * @retval SD_Error：SD 卡错误代码。
  */
...
/**
  * @brief 返回当前 SD 卡的状态。
  * @param psdstatus：指向包含 SD 卡状态的缓冲区的指针 
  *（SD 状态寄存器）。
  * @retval SD_Error：SD 卡错误代码。
  */
...
/**
* @brief 切换模式高速 
* @note 该函数必须在“Transfer State”之后使用
* @note 此操作后面应该是配置 
* PLL 的 SDIOCK 时钟频率介于 67 和 75 MHz 之间
* @参数无
* @retval SD_Error：SD 卡错误代码。
*/
...
/**
  * @brief 为 SDIO Tx 请求配置 DMA2 通道 4。
...
/**
  * @brief 为 SDIO Rx 请求配置 DMA2 通道 4。
