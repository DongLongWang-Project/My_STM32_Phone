...
******************************************************/
void ui_goto_page(UI_APP_ENUM Page,UI_APP_BTN_ENUM APP,UI_APP_SETTING_LIST_ENUM setting_mode)
{
    switch(Page)
    {
	case PAGE_HOME:
		{
			if(app_page!=NULL)lv_obj_del(app_page);/** 删除app列表页**/
	        app_page = NULL;/** 设为空**/
			if(detail_page!=NULL)	lv_obj_del(detail_page);/** 删除详情页**/
			detail_page = NULL;/** 设为空**/
    ...
    case PAGE_APP_LIST:/** 列表页**/
        ...
        switch(APP)
        {
            case btn_list_setting:app_page = page_app_Setting_list_create(tile_main);   break;/**创建wifi **/
            case btn_list_File:	app_page = page_app_list_fat_create(tile_main,Cure_Path);break;/** 创建文件列表**/
        }
        lv_obj_add_flag(home_page, LV_OBJ_FLAG_HIDDEN);/** 隐藏主界面**/
        Cur_Page = PAGE_APP_LIST;/**设置当前为app列表页 **/
        if(detail_page!=NULL)	lv_obj_del(detail_page);/** 删除详情页**/
        detail_page = NULL;/** 设为空**/
        lv_obj_clear_flag(app_page, LV_OBJ_FLAG_HIDDEN);/**app列表页显现 **/
        break;

    case PAGE_APP_DETAIL:/**详情页(page_detail_create函数可以自行设置,只需返回界面就行,传入的对象作为父对象就行) **/
         switch(APP)
        {
            case btn_list_setting:detail_page = page_detail_create(tile_main,setting_mode);  break;
		    case btn_list_File:  detail_page = page_detail_file_create(tile_main,Cure_Path);       break;
            default:
            break;
        }
        lv_obj_add_flag(app_page, LV_OBJ_FLAG_HIDDEN);/**隐藏app列表页 **/
        Cur_Page = PAGE_APP_DETAIL;/**设置当前为详情页 **/
        break;
