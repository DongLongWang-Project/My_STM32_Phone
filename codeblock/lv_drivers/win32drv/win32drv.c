...
static void lv_win32_keypad_driver_read_callback(
    ...
    lv_indev_data_t* data)
{
     WPARAM  cur_key=g_keyboard_value;
//    printf("cur_key: %d \r\n",cur_key); 
    UNREFERENCED_PARAMETER(indev_drv);
    data->state = (lv_indev_state_t)(
    ...
    switch (cur_key)
    {
    case VK_UP:  data->key = LV_KEY_PREV; break;  
    case VK_DOWN: data->key = LV_KEY_NEXT;break;  
...
//    case 93:ui_goto_page(PAGE_HOME, APP_TEMP);break;
//    case 17:btn_back();break;
    default: if (cur_key >= 'A' && cur_key <= 'Z')  cur_key += 0x20;
                data->key = (uint32_t)cur_key; break;
    }
}
...
//                g_keyboard_value = (uint32_t)wParam;
//                break;
    case WM_KEYUP:
    {
    ...
    void* raw_parameter)
{
    ...
    window_class.hCursor = LoadCursorW(NULL,  (const short unsigned int *)IDC_ARROW);
