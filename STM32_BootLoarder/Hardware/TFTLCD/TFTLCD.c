...
void TFTLCD_ShowChar(uint16_t X,uint16_t Y,char Char,const Font_Size *FontSize,uint16_t Color_Penceil )
{
...
void TFTLCD_ShowString(uint16_t X, uint16_t Y, const Font_Size *FontSize, char *String, uint16_t Color_Pencil)
{
   ...
   {
      if(Start_x>LCD_Length || ((LCD_Length-Start_x)<FontSize->Font_Width))
      {
         Start_x=0;
         Start_y+=FontSize->Font_Height;
         if(Start_y>LCD_Height || ((LCD_Height-Start_y)<FontSize->Font_Height))
         {
           TFTLCD_ClearAreas(0,LCD_Length,0,LCD_Height,Background); 
           Start_y=0;
         } 
      }
     TFTLCD_ShowChar(Start_x,Start_y,String[i],FontSize,Color_Pencil); 
     Start_x+=FontSize->Font_Width;
   }
}

void TFTLCD_Printf(uint16_t X, uint16_t Y, const Font_Size *FontSize,uint16_t Color_Penceil,char *format, ...)
{
